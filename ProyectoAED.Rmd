...
df_limpio<-df %>% rename(Situacion_Laboral = `Relación con la actividad económica`) %>% filter(Sexo!="Ambos sexos", Edad !="Total") %>% mutate(Sexo=as.factor(Sexo), Edad=as.factor(Edad), Situacion_Laboral=as.factor(Situacion_Laboral))
```
...
```{r}
levels_sexo<-levels(df_tidy$Sexo)
levels_edad<-levels(df_tidy$Edad)
# La Edad es una variable ordinal, dado que sus variables tienen un orden intrínseco es necesario ordenarlas
orden_edad<-c("De 16 a 19 años","De 20 a 24 años","De 25 a 29 años","De 30 a 34 años","De 35 a 39 años","De 40 a 44 años","De 45 a 49 años","De 50 a 54 años","De 55 a 59 años","De 60 a 64 años","De 65 a 69 años","De 70 y más años")
df_tidy$Edad<-factor(df_tidy$Edad,levels = levels_edad, labels = orden_edad )
