...
```{r}
df<-read_delim(archivo, delim = "\t", escape_double = FALSE, na = "..", trim_ws = TRUE,locale = locale(decimal_mark = ",")) %>% rename(Situacion_Laboral = "Relación con la actividad económica") %>% filter(Sexo != "Ambos sexos",Situacion_Laboral !="Total", Edad != "Total") %>% mutate(Sexo=as.factor(Sexo), Edad=as.factor(Edad),Situacion_Laboral=as.factor(Situacion_Laboral))
```
...
df_final<-df %>% mutate(across(.cols=c(Total),.fns=~replace_na(.,0))) %>% mutate(Situacion_Laboral=recode(Situacion_Laboral,"Parados que buscan primer empleo"="Parados_sin_exp"))
```
