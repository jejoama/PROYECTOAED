...
- Se comprueba la estructura inicial con `r str()`
...
- Se guarda en un dataframe `r df_na` las instancias con *NA*
...
- **Renombrar columnas**: Utilizando `r rename()` para acortar `r "Relación con la actividad económica` por `r "Situacion_Laboral"` para simplificar el manejo de columnas.

- **Filtrado de doble conteo**: Se eliminan las instancias donde las variables representan la suma de otras instancias (`r Sexo="Ambos sexos"` y `r Edad="Total"`)

- **Se aplica `r pivot_wider()`** utilizando los valores de la columna `r Situacion_Laboral` para crear las nuevas columnas (`r names_from`) y los valores numéricos de la columna `r Total`. De esta forma se cumple la regla tidy de una variable por columna.

- **Renombrar variables**: Se renombra la columna `r "parados que buscan primer empleo"` a un formato más corto `r "Parados_sin_exp"`
...
- **Creación de tasas**: Se añaden las columnas `r Tasa_paro`, `r Tasa_actividad` y `r Tasa_ocupacion` utilizando `r mutate()` y las expresiones de la EPA.

- **Ordenación en factores**: Se ordena la columna `r Edad` como un factor ordinal y la columna `r Sexo` como factor nominal.
