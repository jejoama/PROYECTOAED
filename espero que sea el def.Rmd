...
gif <- ggplot(df_anim, aes(x = Sexo, y = Personas, fill = Situacion)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Distribución de activos e inactivos por sexo – Año: {closest_state}",
       x = "Sexo", y = "Personas", fill = "Situación") +
  scale_fill_manual(values = c("Activos" = "red", "Inactivos" = "blue")) +
  theme_minimal() +
...
animacion_actividad <- df_clean %>%
  ggplot(aes(x = Edad, y = Tasa_actividad, color = Sexo, group = interaction(Sexo, Edad))) +
...
```{r}
df_largo <- df_clean %>%
  pivot_longer(cols = c(Ocupados, Parados, Inactivos), names_to = "Situacion", values_to = "Personas")

tabla <- table(df_largo$Sexo, df_largo$Situacion)
chisq.test(tabla)
